...
For instance, in the mathematical library for the Lean theorem prover, \mathlib~\cite{lean-prover},
...
The utility of the \lean{ring} tactic is evident from the fact that it is invoked over 300 times in \mathlib. % TODO: reword this sentence!
...
The result is that \ring cannot prove $2^{n+1} - 1 = 2 * 2^n - 1$.
...
Both functions additionally return a proof showing the input and output expressions are equal.
...
and the right argument to \lean{exp} is an \lean{ex prod}.
